%Plot Figurs for post contraction
%plot figures
clear
addpath("Functions\")
...
title(h(1),'\textbf{A. cTBS300}', '\textbf{with post contraction}',...
    'Interpreter','latex','FontSize',titilefont)
title(h(2),'\textbf{B. iTBS600}', '\textbf{with post contraction}',...
    'Interpreter','latex','FontSize',titilefont)


lg = legend(h(2),[p1,p2,p3,p4],'Interpreter', 'latex','FontSize',legendfont,...
    'Location','northeast');
set(lg.BoxFace, 'ColorType','truecoloralpha', 'ColorData',uint8(255*[1;1;1;0.75]));


%t2 setting
ylabel(h(3),{'\textbf{Stage III}','\textbf{After-effect}'},'FontSize',labelfont,...
...
lg = legend(h(4),[p5,p6,p7],'Interpreter', 'latex','FontSize',legendfont);
set(lg.BoxFace, 'ColorType','truecoloralpha', 'ColorData',uint8(255*[1;1;1;.75]));

%export images
exportgraphics(gcf,'AfterEffect with post contraction.pdf','ContentType','vector');
